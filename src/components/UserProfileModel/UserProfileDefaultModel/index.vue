<template>
    <div ref="avatarRef" class="relative">
        ...
        class="cursor-pointer" :src="userStore.getUserProfile.avatar" />
        <div class="flex flex-col h-auto w-70 absolute space-y-8 right--32 top-13 bg-[white] rounded-2 shadow-xl border-1 border-[#E3E5E7] p-y-3 p-x-4"
            v-if="showModel">
            <div class="flex flex-col items-center justify-center space-y-5 mt-2">
                <el-avatar size="large" class="cursor-pointer" :src="userStore.getUserProfile.avatar" />
                <!-- 用户昵称 -->
                <span class="text-xl font-medium color-[gray]">{{ userStore.getUserProfile.nickName }}</span>
                <!-- 等级条 -->
            ...
            <div class="flex flex-row items-center justify-center space-x-15">
                ...
                <div class="flex flex-col space-y-1 items-center justify-center">
                    <span class="text-base font-bold">{{ userStore.getUserProfile.follow }}</span>
                    <span class="text-xs font-light color-[#9499A0]]">关注</span>
                </div>
                <!-- 粉丝 -->
                <div class="flex flex-col space-y-1 items-center justify-center">
                    <span class="text-base font-bold">{{ userStore.getUserProfile.fans }}</span>
